...
if (new Set(ids).size !== 100) throw new Error('Duplicate tool id detected.');
if (new Set(slugs).size !== 100) throw new Error('Duplicate tool slug detected.');
for (let id = 1; id <= 100; id += 1) {
  if (!ids.includes(id)) throw new Error(`Missing tool id ${id}`);
}
...
for (const tool of TOOLS) {
  ...
  for (const field of tool.fields) {
    ...
    if (field.type === 'select' && (!Array.isArray(field.options) || field.options.length < 1)) {
      throw new Error(`Missing select options: ${tool.id}/${field.key}`);
    }
  }
  if (tool.operation === 'template' && !tool.template) throw new Error(`Missing template: ${tool.id}`);
  if (tool.operation === 'checklist' && (!Array.isArray(tool.checklist) || tool.checklist.length < 1)) {
    throw new Error(`Missing checklist: ${tool.id}`);
  }
  ...
  if (tool.mode !== 'image') {
    const values = Object.fromEntries(
      tool.fields.map((field) => [
        field.key,
        field.type === 'checkbox' ? Boolean(field.sample) : field.sample ?? ''
      ])
    );
...
for (const operation of new Set(TOOLS.map((tool) => tool.operation))) {
  if (!operationsSource.includes(`'${operation}'`)) {
    throw new Error(`Operation is not connected in operations.js: ${operation}`);
  }
}
